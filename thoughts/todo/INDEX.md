...
| --- | --- | --- |
| [No logout path revokes a refresh token](logout-does-not-revoke-refresh-chain.md) | Makes "log out" end the session **server-side**, not just on the device: today the access token is revoked and the refresh chain lives out its full TTL (14 days at the consumer). Wider than filed — the cookie-revoking branch is dead code, so the web SPA is in the same state as mobile | Semantics are settled (chain-scoped, per the `create_refresh_token` design); the *storage* cost is not — `delete_by_chain` on DynamoDB scans the whole shared token partition, which its own docstring licenses only for a rare theft event. Three options in the file; pick one before implementing. **A cheap half is independent of all that**: `docs/guides/spa-authentication.rst:207` advertises `/oauth/logout` as clearing "all tokens", which is false for every client shape, and the guide never mentions the `/oauth/revoke` workaround |
