...
| --- | --- | --- | --- |
| 22 | [Scoped bulk list reads + `get_attr()` after `get_bucket()`](scoped-bulk-list-reads.md) | **Item A:** a bulk list read can be scoped to a **list-name prefix** instead of always dumping the partition — measured on a consumer's heaviest account at **1,361.0 RCU / 11 chained pages → 685.0 RCU / 8 queries in 5 chains**, issuable as five concurrent streams instead of one eleven-page chain (**1,224 ms serial → 700 ms concurrent**). **Item A3**, found on review and needing no public API: `verify()`/`compact()`/`migrate_to_v2()` each dump the whole partition to read **one** list, which the maintenance scripts multiply to ~**2N + 1 dumps** for N lists. **Item B:** `get_attr()` stops issuing a point read for a name it can already prove absent, and stops polluting the loaded bucket with keys that have no row | **A is the follow-on 3.14 named and deferred.** `thoughts/plans/2026-08-20-v2-positional-access-cost.md` made exactly this change for the plain-property path and scoped the bulk path out because "it legitimately wants the whole partition" — true then, and this is the caller that makes it no longer true. Both items are **additive and patch-releasable** — no next-major dependency. **Two traps found on verification, neither in the original filing:** `get_range`'s `consistent_read` defaults to `True` while today's dump is eventually consistent, so a naive reuse costs ~1,370 RCU and is a *regression*; and item B's fix inherits DynamoDB's over-matching `get_bucket()` (`startswith(bucket)`, not `bucket + ":"`), which must be fixed with it. Six open decisions are laid out in the research doc §7. Evidence, provenance and corrections: `thoughts/research/2026-08-29-bulk-list-reads-from-a-consumer.md` |
