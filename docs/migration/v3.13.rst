...
If your items carry an identifying field, pass it::
    ...
    poetry run python scripts/verify_property_lists.py --identity-key id
...
Step 4: understand the new storage format
-----------------------------------------
...
**Lazy migration.** A healthy v1 list of 50 items or fewer migrates on its
next mutation. Two deliberate limits:
...
- **You can turn it off.** Migration is inline and synchronous — one
  ``append()`` to a 40-item list performs the entire migration inside that
  request. Set ``ACTINGWEB_LAZY_MIGRATION_MAX_LENGTH=0`` to keep it out of
  user traffic entirely, or raise it above 50 to let larger lists migrate
  inline. On Lambda or any latency-sensitive deployment, ``0`` plus a
  scheduled sweep is the safer shape.

**Bulk migration**, for lists too large or too idle to migrate lazily, and
the only route for damaged lists once you have repaired them::

    poetry run python scripts/migrate_property_lists.py            # dry run
    poetry run python scripts/migrate_property_lists.py --migrate

``migrate_to_v2()`` is idempotent and safe to interrupt and re-run.
