...
.. danger::
   ...
   **Set ``ACTINGWEB_LAZY_MIGRATION_MAX_LENGTH=0`` for this release's first
   deployment.** Then no list changes format, and rolling back stays a pure
   code rollback with no data to reconcile. Migrate later, deliberately.
...
- **You can turn it off** — see step 4, which is the reason you should for
  a first deployment. Migration is also inline and synchronous: one
