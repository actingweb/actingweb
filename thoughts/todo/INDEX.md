...
| --- | --- | --- | --- |
| 22 | [Scoped bulk list reads + `get_attr()` after `get_bucket()`](scoped-bulk-list-reads.md) | **Item A:** a bulk list read can be scoped to named families instead of always dumping the partition — measured on a consumer's heaviest account at **1,361 RCU / 11 sequential pages → 686 RCU / 8 independent queries**, and the 8 have no pagination chain so they parallelise (**1,224 ms serial → 700 ms concurrent**). **Item B:** `get_attr()` stops issuing a point read for a name it can already prove absent | **A is the follow-on 3.14 named and deferred.** `thoughts/plans/2026-08-20-v2-positional-access-cost.md` made exactly this change for the plain-property path and scoped the bulk path out because "it legitimately wants the whole partition" — true then, and this is the caller that makes it no longer true. `get_range` already exists on the protocol and both backends, so nothing is needed at the database layer; the sentinel and collation traps are already documented from that plan. Both items are **additive and patch-releasable** — no next-major dependency. Evidence and provenance: `thoughts/research/2026-08-29-bulk-list-reads-from-a-consumer.md` |
