...
| --- | --- | --- |
| [`POST /oauth/email` rewrites existing actors; `actor_created` fires twice](oauth-email-form-writes-to-existing-actors.md) | Stops the email-entry form selecting any existing actor by typed address and re-running the consumer's `actor_created` hook on it (the consumer's hook un-suspends and re-grants a trial), and stops the hook firing **twice** for every new OAuth actor, which it has done since v3.5.1 with no test noticing. Also carries the eleven non-constant-time secret compares, the check-then-create creator race, and the two email case rules | Two decisions come first, both small: whether the self-asserted email flow survives at all (its stated purpose targets a case GitHub itself refuses to authorise; what it catches today is a `/user/emails` failure), and whether `actor_created` fires in core or at handler level. Both are laid out in `thoughts/research/2026-09-09-oauth-email-form-writes-to-existing-actors.md`. The consumer plans a 404 shadow of the route, so its exposure closes without us; every other `require_email` consumer stays exposed |
