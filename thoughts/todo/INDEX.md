...
| 14 | **Positional list reads are quadratic under v2** — the corrections on item 2 in [DynamoDB known-next](dynamodb-known-next.md), with the 2026-08-19 consumer incident | Closes the cost defect the v2 migration introduced while closing v1's corruption defect: `_v2_getitem` forces a **whole-list Query** before every positional read, so `for i in range(len(lst)): lst[i]` is O(n) whole-list reads and a *k*-item delete loop is O(k·n) | **The register's trigger fired.** The reference consumer took a user-visible outage on 2026-08-19 — a 10-item delete ran **40.7 s** (4.5 s under v1), was cut off by a 30 s gateway ceiling, and burned ~1.19M RCU for ~67 deletions. It fixed its own caller, so nothing in-library changed. Note the item's stated remedy is **unreachable**: `PropertyListStore.__getattr__` mints a fresh `ListProperty` per access, so no instance cache survives. `__iter__`/`to_list`/`slice`/`to_indexed_list` are all already one query — the exposure is the explicit positional loop alone, which is cheap to mark. The primitive that would remove the loop entirely is row 15 |
| 15 | [Address list items by identity, not position](list-delete-by-value-primitive.md) — designed in [research 2026-08-20](../research/2026-08-20-v2-positional-access-cost.md) | Removes the reason consumers derive an index at all. **v2 already has stable per-item identity — the rank — and the API hides it**, so every mutation re-derives a fragile handle from a stable one, and re-derivation *is* the whole-list read. Exposing it closes row 14's cost **and** makes the index-skew class that destroyed a neighbour row on 2026-07-19 unrepresentable | **Mostly composition** — `_v2_load_full()` resolves every id in one range read; `delete_if_value_equals` ships on both backends and `_v2_pop` already uses the exact resolve-read-conditional-delete-retry shape. Cost: *k* × (2 range reads + delete) → 1 range read + *k* point writes. Two constraints are now established: `BatchWriteItem` cannot express conditions, and a rank is unique only within a **list generation**, so handles must be mutated conditional on the bytes read. `remove_where` becomes a convenience on top. **Postgres answered 2026-08-20** ([research](../research/2026-08-20-v2-cost-in-library-callers.md)): the delete half composes today on both backends, but `update_by_handle` needs a **conditional set, which no backend has** — the protocol carries a conditional create and a conditional delete and nothing else, so the two halves need not ship together. Open: the v1 answer |
| 16 | [The library's own callers pay the v2 list cost](library-callers-pay-v2-list-cost.md) — verified in [research 2026-08-20](../research/2026-08-20-v2-cost-in-library-callers.md) | Removes the outage shape from **our own documented endpoint**: `POST /{actor}/properties` with `{"items": […]}` costs ≈**`2k + 2` whole-list Queries** for a *k*-item request, so a consumer reaches the 2026-08-19 failure by following the docs rather than by writing a positional loop. Also stops the built-in web UI spending a whole-list read per count badge, and stops **two** eagerly-evaluated `len()` calls running per web-UI append at any log level | **The cheapest item in the v2 cost cluster and the only one needing no new API** — it is the same caller-side fix the reference consumer already shipped for itself, and every fix shape already exists in the codebase (`to_indexed_list()`, `prime_from_rows`, and `www.py:353-354`'s correct count). Ships in a patch release without settling any row-15 decision. Carries the docs correction with it: the guide's *"two queries per item"* reads as a constant factor when the first query is whole-list, and `len()`/`append()` costs are undocumented. One open question only — whether the bulk *write* loop is fixed now or once, later, since row 15 would rewrite it again |
...
| 17 | [`AuthenticatedPropertyListStore.create()`/`.delete()` raise `TypeError`](authenticated-list-store-create-delete-broken.md) | Fixes two exported public methods that have **never worked**: both resolve through `__getattr__` to a `NotifyingListProperty` and raise `TypeError: not callable` after the permission check passes. `delete(name)` has a real implementation behind it (`ListProperty.delete()`); the wrapper just reaches for it wrongly | Small, self-contained, and **nothing to do with v2** — introduced 2025-12-14, so do not sequence it behind the cost work it was found alongside. No callers, no tests, no docs anywhere in the repo, which is exactly why eight months passed; whoever fixes it should add the test. It sits on the *permission-enforcing* view, so the only people who can hit it are app developers least placed to diagnose an error naming an internal type |
