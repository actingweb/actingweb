...
measurements. Three items surfaced while implementing it and are filed
below as rows 23, 24 and 25. This section is otherwise empty.
...
| 23 | [`_glob_to_regex()` anchoring gaps](glob-to-regex-anchoring-gaps.md) | Closes two newline holes in permission pattern matching: `$` also matches before a trailing newline, so pattern `notes` matches target `"notes\n"`; and `.` excludes newline without `re.DOTALL`, so `memory_*` does **not** match `"memory_a\nb"` — a namespace rule that silently does not cover part of its namespace | Equally broken before v3.14.3 in the single-list path, where the target came from the CALLER. What changed is **reachability**: the authenticated bulk readers added in that release pass names that came from **storage**, so a list whose name contains a newline is now matched without anyone having typed it. The fix is `\\Z` and `re.DOTALL` — two characters and a flag — but it changes matching semantics for every permission rule in every deployment, which wants its own tests and release note rather than riding in a patch |
| 24 | [Attribute upsert bucket drift](attribute-upsert-bucket-drift.md) | Removes a real "same input, different stored state" difference between the backends: on a colliding composite key DynamoDB's PutItem reattributes the row to the last writer while PostgreSQL's `ON CONFLICT DO UPDATE` keeps the first writer's `bucket` | Found while writing v3.14.3's Phase 1 tests. After that release both backends agree on the part that matters — the row answers to exactly one bucket, never both — so this is tidiness, not safety. Reaching the collision needs one bucket name to be a prefix of another AND an attribute name equal to the remainder. Fix 1 (add `bucket`/`name` to the PostgreSQL SET list) is one line; fix 2 (a delimiter that cannot appear in either half) is a key-layout change needing a migration, and belongs with `prop-list-key-prefix-scheme.md` |
| 25 | [`get_bucket()` empty-vs-`None` contract](attribute-get-bucket-empty-vs-none.md) | Makes `DbAttribute.get_bucket()` mean the same thing on both backends: `{}` for empty, `None` only for a caught exception. Today PostgreSQL returns `None` for both, so since v3.14.3 an empty bucket there is never trusted and `get_attr()` still point-reads on it | Deferred from v3.14.3's Phase 6a on purpose — the conservative version cannot break a caller a grep did not find, and the cost is one point read on an empty bucket. Pinned by an integration assertion that flips when this lands. One backend edit plus one test change, wanting its own release note |
...
| # | Item | Waiting on |
| --- | --- | --- |
| 3 | Postgres parallel DELETE not persisting — **closed 2026-09-02, file deleted.** Kept as a ledger row because rows 10 and §0 cite it. Quarantine lifted and path instrumented in #128 (2026-08-15); green on every run through the v3.14.3 tag with zero reruns. The evidence is in git history before 2026-09-02; `ACTINGWEB_PG_DELETE_DIAGNOSTICS` stays on in `tests.yml` so a recurrence names its mechanism | Nothing — unless it recurs, in which case reopen from the git history, not from memory |
...
| 20 | [AI agent discoverability follow-ups](ai-agent-discoverability-followups.md) | Three verification steps the `ai-agent-discoverability` plan (`status: done`) couldn't complete in-session: submit to Context7, connect a real MCP client to the new quickstart, install the Agent Skill in a scratch consumer repo. The fourth — confirm the readthedocs.io build — was **done 2026-09-02**: both quickstarts render at the `docs/`-prefixed URLs every advertised link uses | Item 2 waits on someone with a Context7 account (a 200 from that host proves nothing, it 200s for made-up paths too); items 3-4 on resources this environment doesn't have — a real OAuth2 provider/MCP client and a throwaway consumer repo |
