...
def _diag_probe(tag: str, actor_id: str, client_id: str, actor_interface: Any) -> None:
    ...
    try:
        try:
            ...
            n = -1
            peers = [f"err={e}"]
        cfg = getattr(getattr(actor_interface, "_core_actor", None), "config", None)
        logger.error(
            "TMPDIAG %s pid=%s tid=%s actor=%s client=%s n_via_api=%s %s cfg=%s db=%s peers=%s",
            tag,
            os.getpid(),
            threading.get_ident(),
            actor_id,
            ...
            _pg_state(actor_id),
            id(cfg),
            getattr(getattr(cfg, "DbTrust", None), "__name__", None),
            peers[:4],
        )
