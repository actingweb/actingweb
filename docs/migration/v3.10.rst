...
SPA Flow: Email Form Fallback
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When GitHub returns no verified emails and ``require_email=True``, the SPA OAuth callback
now redirects to ``/oauth/email`` (the email collection form) instead of returning a hard
error. This matches the existing Web UI behavior. The email form flow:

1. Collects the user's email address
2. Creates the actor with ``email_verified = "false"``
3. Fires the ``email_verification_required`` lifecycle hook
4. The application can then send a verification email
