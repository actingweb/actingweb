...
# Look for psycopg ConnectionPool metrics
```
...
|-----------|----------|------------|-------|
| Single actor read | 5-10ms | 1-3ms | PostgreSQL faster (local) |
| Property read | 5-10ms | 1-2ms | PostgreSQL faster |
| Trust query | 10-20ms | 2-5ms | PostgreSQL faster (JOIN support) |
| Batch operations | Good | Excellent | PostgreSQL supports transactions |
| Full table scan | Poor | Good | PostgreSQL has better filtering |
| Concurrent writes | Excellent | Good | DynamoDB has better write scaling |
