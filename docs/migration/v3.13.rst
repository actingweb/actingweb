...
.. warning::
   ...
   **What "not taking writes" does and does not buy you.** Nothing excludes
   ...
   longer corrupt the list's **format**: a concurrent write updates only the
   metadata fields it changes, merged into a fresh read, so it can never put
   ``format: 1`` back over a completed migration. In ``rc6`` and earlier it
   could, and the result was total silent loss — see the CHANGELOG.
